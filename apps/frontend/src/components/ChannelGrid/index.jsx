...
const ChannelGrid = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setContent(<Loader />);
      const children = await GetChildrenController.getAllItemsById(CHANNEL, user_id);
      console.log(children);
      if (children.length === 0) {
        setContent(<p style={{color: 'white'}}>Не создано ни одного Канала </p>);
      } else {
  ...
  const handleCreateChild = () => {
    ...
    navigate(`/${CHANNEL}/create/${user_id}`, { state: { idList: location.state.idList } });
  }
